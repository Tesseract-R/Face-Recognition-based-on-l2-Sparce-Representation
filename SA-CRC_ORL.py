...
def cs_omp(y, D):
    ...
    residual = y  # 初始化残差
    index = np.zeros((ylen), dtype=int)  # 选出原子的索引集
    result = np.zeros((img_train_norm.shape[1]), dtype=float)  # 待求的系数
    for i in range(ylen):
        index[i] = -1
    for j in range(ylen):  # 迭代次数
...
for test_time in range(img_test_norm.shape[0]):
    ...
    x_up = np.dot(np.dot(x_up, img_train_norm.T), y)
    coefficient_x = (x_up + x_down)/np.linalg.norm(x_up + x_down)
    #for i in range(1207):
