...
path = open('./image/ORL_Faces/train.txt', 'r')
img_train = np.array([])
...
for line in path:
    ...
    img = cv2.pyrDown(cv2.pyrDown(np.array(img)))  # 降采样 倍数为4
...
path = open('./image/ORL_Faces/test.txt', 'r')
img_test = np.array([])
...
def cs_ADM(y, D):
    ...
    while True:
        ...
        if loop_time > 20:
            break
        if np.linalg.norm(residual_1, ord=np.inf) <= 0.01 and np.linalg.norm(residual_2, ord=np.inf) <= 0.1:
            break
    result = alpha   #稀疏系数
    return result
